...
const requiredText = (max: number) => S.String.pipe(S.maxLength(max), S.filter(s => s.trim().length > 0));
const text = (max: number) => S.String.pipe(S.maxLength(max));
const link = text(2000).pipe(S.filter(value => {
...
export const RoutineEditSchema = S.Struct({ ...RoutineSchema.fields, version: S.Number.pipe(S.int(), S.positive()) });
