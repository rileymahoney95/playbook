    ...
    <>
      ...
      {data?.items.length === 0 && (
        <div className="empty">
          ...
          <p className="muted mt-2">Completed and discarded runs will appear here.</p>
      ...
      <div className="grid gap-3">
        {data?.items.map((run) => (
          <Link className="card history-row" key={run.id} to={`/runs/${run.id}`}>
            <div className="min-w-0">
              <h2 className="text-base">{run.title}</h2>
              <p className="muted text-sm mt-1">
                {dateTime(run.finishedAt!)} · {run.completedSteps} of {run.stepCount} steps
              </p>
            </div>
            <span className={`badge ${run.status === 'completed' ? 'completed' : ''}`}>
              {run.status === 'completed' ? 'Completed' : 'Discarded'}
            </span>
          </Link>
        ))}
      </div>
      {(page > 0 || data?.hasMore) && (
        <div className="flex justify-between items-center mt-6 gap-3">
          ...
          <span className="muted text-sm">Page {page + 1}</span>
          <button
            className="button secondary"
