...
if (!new URL(url).pathname.endsWith('_test')) throw new Error('Tests require a dedicated database ending in _test.');
...
let cookie = '';
async function call(path: string, method = 'GET', body?: unknown, session = cookie, extra: Record<string, string> = {}, ip = 'test') {
  const response = await app(new Request(`http://localhost:3000${path}`, {
    method, headers: { 'Content-Type': 'application/json', 'X-Playbook-Request': '1', Cookie: session, ...extra },
    body: method === 'GET' ? undefined : JSON.stringify(body ?? {}),
  }), ip);
  return { status: response.status, data: await response.json() as any, headers: response.headers };
}
const sample = () => ({ title: 'Test routine', description: 'Test description', category: 'Maintenance', steps: [
  { title: 'First step', instructions: 'Keep this snapshot', quantity: '3 reps', durationSeconds: 2, referenceUrl: 'https://example.com' },
  { title: 'Second step', instructions: '', quantity: '', durationSeconds: null, referenceUrl: '' },
] });
async function create() { const result = await call('/api/routines', 'POST', sample()); expect(result.status).toBe(201); return result.data.id as string; }
async function start(id: string) { const result = await call(`/api/routines/${id}/runs`, 'POST'); expect(result.status).toBe(200); return result.data.id as string; }
async function getRun(id: string) { const result = await call(`/api/runs/${id}`); expect(result.status).toBe(200); return result.data as Run; }
...
describe('Playbook real-Postgres integration', () => {
  ...
  test('rejects cross-origin writes, missing CSRF header, and invalid payloads', async () => {
    expect((await call('/api/routines', 'POST', sample(), cookie, { Origin: 'https://evil.example' })).status).toBe(403);
    expect((await call('/api/routines', 'POST', sample(), cookie, { 'X-Playbook-Request': '' })).status).toBe(403);
    ...
    const invalid = sample(); invalid.steps[0].referenceUrl = 'javascript:alert(1)';
    expect((await call('/api/routines', 'POST', invalid)).status).toBe(400);
    invalid.steps[0].referenceUrl = ''; invalid.steps[0].durationSeconds = 0;
    expect((await call('/api/routines', 'POST', invalid)).status).toBe(400);
  ...
  test('persists checks across sessions and prevents stale writes', async () => {
    ...
    const check = await call(`/api/runs/${id}/steps/${first.id}`, 'PATCH', { version: first.version, completed: true });
    ...
    expect((await call(`/api/runs/${id}/steps/${first.id}`, 'PATCH', { version: first.version, completed: false })).status).toBe(409);
  ...
  test('edits, reorder, add and removal affect future runs but preserve snapshots', async () => {
    ...
    const input = { ...routine, title: 'Updated template', steps: [{ ...routine.steps[1], title: 'New first step' }] };
  ...
  test('timers persist start, elapsed time, pause and reset', async () => {
    ...
    const started = await call(`/api/runs/${id}/steps/${step.id}`, 'PATCH', { version: step.version, timerAction: 'start' });
    expect(started.status).toBe(200);
    expect(started.data.steps[0].timerStartedAt).not.toBeNull();
    await Bun.sleep(100);
    step = (await getRun(id)).steps[0];
    const paused = await call(`/api/runs/${id}/steps/${step.id}`, 'PATCH', { version: step.version, timerAction: 'pause' });
    ...
    const reset = await call(`/api/runs/${id}/steps/${step.id}`, 'PATCH', { version: step.version, timerAction: 'reset' });
    expect(reset.data.steps[0].timerRemainingMs).toBe(2000);
    const untimed = reset.data.steps[1];
    expect((await call(`/api/runs/${id}/steps/${untimed.id}`, 'PATCH', { version: untimed.version, timerAction: 'start' })).status).toBe(400);
  });
  ...
  test('completion requires all checks, is idempotent, and starts fresh next time', async () => {
    ...
    for (const step of run.steps) expect((await call(`/api/runs/${id}/steps/${step.id}`, 'PATCH', { version: step.version, completed: true })).status).toBe(200);
    const finished = await call(`/api/runs/${id}/complete`, 'POST');
    expect(finished.data.status).toBe('completed');
    expect((await call(`/api/runs/${id}/complete`, 'POST')).data.finishedAt).toBe(finished.data.finishedAt);
    run = await getRun(id);
    expect((await call(`/api/runs/${id}/steps/${run.steps[0].id}`, 'PATCH', { version: run.steps[0].version, completed: false })).status).toBe(409);
    const nextId = await start(routineId);
    expect(nextId).not.toBe(id);
    expect((await getRun(nextId)).completedSteps).toBe(0);
    expect((await call('/api/history')).data.items.some((r: Run) => r.id === id && r.status === 'completed')).toBe(true);
  });
  ...
  test('records cannot be addressed through the wrong parent', async () => {
    const a = await start(await create()); const b = await start(await create());
    const step = (await getRun(a)).steps[0];
    expect((await call(`/api/runs/${b}/steps/${step.id}`, 'PATCH', { version: step.version, completed: true })).status).toBe(404);
    expect((await getRun(a)).completedSteps).toBe(0);
  });

  test('rate limits password attempts', async () => {
    for (let i = 0; i < 10; i++) expect((await call('/api/login', 'POST', { password: 'wrong' }, '', {}, 'throttle-test')).status).toBe(401);
    expect((await call('/api/login', 'POST', { password }, '', {}, 'throttle-test')).status).toBe(429);
  });

  test('password changes revoke old sessions and seeding never overwrites the password', async () => {
    const changed = await call('/api/password', 'POST', { currentPassword: password, newPassword: 'changed-test-password-987!' });
    ...
    await seedOwner(db, password);
    expect((await call('/api/login', 'POST', { password: 'changed-test-password-987!' }, '', {}, 'new-password')).status).toBe(200);
    expect((await call('/api/login', 'POST', { password }, '', {}, 'new-password')).status).toBe(401);
