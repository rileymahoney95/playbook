...
export function createAuth(db: Database, secure: boolean) {
  ...
  const tokenFrom = (request: Request) =>
    request.headers
      ...
      ?.split(';')
      .map((x) => x.trim())
      .find((x) => x.startsWith(`${cookieName(secure)}=`))
      ?.split('=')[1];
  ...
  const authenticate = (request: Request) =>
    Effect.gen(function* () {
      ...
      if (!token || !/^[A-Za-z0-9_-]{43}$/.test(token))
        return yield* Effect.fail(new AppError({ status: 401, message: 'Please sign in.' }));
      const [session] = yield* task(() =>
        db
          .select()
          .from(sessions)
      ...
      if (!session)
        return yield* Effect.fail(
          new AppError({ status: 401, message: 'Your session expired. Please sign in again.' }),
        );
  ...
  const throttle = (ip: string) =>
    task(async () => {
      ...
      const [attempt] = await db
        ...
        .values({ key, expiresAt: new Date((window + 1) * 600_000) })
        .onConflictDoUpdate({
  ...
  return {
    authenticate,
    login: (password: string, ip: string) =>
      Effect.gen(function* () {
        yield* throttle(ip);
        const [owner] = yield* task(() => db.select().from(owners).limit(1));
        const valid =
          owner && (yield* task(() => Bun.password.verify(password, owner.passwordHash)));
        if (!valid)
          return yield* Effect.fail(new AppError({ status: 401, message: 'Incorrect password.' }));
        ...
        return yield* task(() =>
          db.transaction(async (tx) => {
            const [current] = await tx
              ...
              .for('update');
            if (current.passwordHash !== owner.passwordHash)
              throw new AppError({
                status: 401,
    ...
    changePassword: (ownerId: string, currentPassword: string, newPassword: string, ip: string) =>
      Effect.gen(function* () {
        yield* throttle(ip);
        const passwordHash = yield* task(() =>
          Bun.password.hash(newPassword, { algorithm: 'argon2id' }),
        );
        return yield* task(() =>
          db.transaction(async (tx) => {
            const [owner] = await tx
              ...
              .for('update');
            if (!owner || !(await Bun.password.verify(currentPassword, owner.passwordHash)))
              throw new AppError({ status: 400, message: 'Current password is incorrect.' });
