...
export const routineSteps = pgTable('routine_steps', {
  ...
  check('routine_step_duration', sql`${t.durationSeconds} IS NULL OR ${t.durationSeconds} BETWEEN 1 AND 86400`),
]);
...
export const runs = pgTable('runs', {
  ...
  uniqueIndex('one_active_run_per_routine').on(t.routineId).where(sql`${t.status} = 'active'`),
...
export const runSteps = pgTable('run_steps', {
  ...
  check('run_step_duration', sql`${t.durationSeconds} IS NULL OR ${t.durationSeconds} BETWEEN 1 AND 86400`),
